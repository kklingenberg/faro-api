...
module.exports = {
  ...
  maxLength: (...pairs) => args => {
    for (let [field, length] of pairs) {
      if (typeof args[field] === "string" && args[field].length > length) {
        return Promise.reject(
          errors.badParameters(
            `string field '${field}' exceeds maximum length`,
            {
              maxLength: length,
              given: args[field].length,
              field
            }));
      }
    }
